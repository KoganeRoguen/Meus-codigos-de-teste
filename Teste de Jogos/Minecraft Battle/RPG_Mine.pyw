...
def use_sword():
    ...
    if ed_hp > 0:
        ...
        if ed_hp <= 0:
            ...
            bow_button.destroy()
            gapple_button.destroy()
        elif p_hp <= 0:
            status_label.config(text="\nYou've been defeated by the Ender Dragon. You Lost :(")
            sword_button.destroy()
            bow_button.destroy()
            gapple_button.destroy()
def use_bow():
...
def use_gapple():
    ...
    gapple_heal = random.randint(10, 35)
    ed_damage = random.randint(5, 30)
...
gapple_button = Button(root, text="Golden Apple", command=use_gapple)
gapple_button.pack()
